# INI AWS Secret Access Key Hardcoded

**ID:** `INI_AWS_SECRETACCESSKEY_HARDCODED`
**Severity:** CRITICAL

## Description

An AWS Secret Access Key was found hardcoded in an INI configuration file. Hardcoding AWS credentials in configuration files is a major security risk. If this file is accidentally exposed or committed to a version control system, an attacker could gain programmatic access to your AWS account, potentially leading to data theft, resource hijacking (e.g., for crypto-mining), or complete infrastructure compromise.
...
1.  **Immediately Rotate the Exposed Credentials:** Assume the exposed credentials have been compromised. Log into your AWS account, deactivate the exposed secret access key, and generate a new one.
